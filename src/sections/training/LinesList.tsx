...
export function LinesList() {
    ...
    const onExport = useCallback(() => {
        ...
        if (!ok) {
            setReport({
                tone: 'error',
                headline: 'The browser refused the download.',
                details: [],
            });
        }
    }, [folder]);
    ...
    const confirmRename = useCallback(() => {
        ...
        const target = normaliseFolderPath(
            parent.length === 0 ? renameTo : `${parent}/${renameTo}`,
        );
        ...
        <Wrap data-testid="lines-list" data-folder={folder}>
          ...
          guess. The last crumb is the open folder and is not a link. */}
                ...
                <Body>
                    ...
                    {!storeEmpty && (
                        <SearchRow>
                            <SearchLabel htmlFor="lines-search">
                                {inFolder
                                    ? `Search by name in ${folderName(folder)}`
                                    : 'Search by name'}
                            </SearchLabel>
                            <SearchInput
                                id="lines-search"
                            ...
                            />
                            {inFolder && <Hint>Searches this folder and everything below it.</Hint>}
                        </SearchRow>
                    )}
                    ...
                    {storeEmpty ? (
                        <EmptyState data-testid="lines-empty">
                            ...
                            <p>
                                A line is a starting position plus one ordered sequence of moves —
                                the repertoire you then practise in drill. Give a line a folder (
                                <code>Black/Sicilian</code>) to group it with the rest of that
                                repertoire.
                            </p>
                            <Button
                                variant="primary"
                        ...
                        matches.length === 0 ? (
                            ...
                            <Rows>{matches.map((line) => renderRow(line))}</Rows>
                        )
                    ) : (
                        <>
                            {subfolders.length > 0 && (
                  ...
                  the last line moved out. Folders have no existence of their own to report on. */}
                            {nothingHere && (
                                <EmptyState data-testid="folder-empty">
                                    <p>
                                        There is nothing in {folderName(folder)}. Its lines may have
            ...
            {pendingDelete !== null && (
                ...
                >
                    <strong>
                        {pendingDelete.name.trim().length === 0
                            ? 'Untitled line'
                            : pendingDelete.name}
                    </strong>{' '}
                    ({moveCountLabel(pendingDelete.moves.length)}) will be removed. This cannot be
                ...
                >
                    <ResetBody>
                        <p>
                            ...
                            <strong data-testid="reset-total">
                                {lineCountLabel(pendingReset.total)}
                            </strong>
                        ...
                        <p>
                            {pendingReset.custom === 0 ? (
                                <>
                                    None of them is your own — every stored line comes from the
                                    bundled repertoires and is added again the next time the app
                                    loads.
                                </>
                            ) : (
                                <>
                                    <strong data-testid="reset-custom">
                                        {lineCountLabel(pendingReset.custom)}
                                    </strong>{' '}
                                    {pendingReset.custom === 1 ? 'was' : 'were'} created, imported
                                    or edited by you and cannot be recovered. The rest come from the
                                    bundled repertoires and are added again the next time the app
                                    loads.
                                </>
                            )}
                        </p>
            ...
            {renameTo !== null && (
                ...
                >
                    <DialogField>
                        ...
                        />
                        <span>
                            {lineCountLabel(scope.length)} will move with it, subfolders included.
            ...
            >
              ...
              results from a subfolder. */}
                ...
                <Actions>
                    ...
                    <Button
                        size="sm"
                        data-testid="line-explore"
                        onClick={() => openInExplore(line)}
                    >
                        Explore
                    </Button>
                    <LinkButton
                    ...
                    <Button
                        size="sm"
                        data-testid="line-analyze"
                        onClick={() => openInAnalyze(line)}
                    >
                        Open in analyze
                    </Button>
                    <Button
