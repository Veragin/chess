...
export function PositionEditor({
  ...
  const endDrag = useCallback(() => {
    ...
    setGhost(null);
    setHover(null);
    if (record?.capture instanceof Element && record.capture.hasPointerCapture(record.pointerId)) {
  ...
  const handlePointerUp = useCallback(
    (event: ReactPointerEvent<HTMLElement>) => {
      ...
      if (!record.moved) {
        ...
        if (tool === null) return;
        commitModel(
          tool.kind === 'erase' ? removePiece(model, from) : placePiece(model, from, tool.piece),
        );
        return;
      }
  ...
  const problem = fenError ?? (validation.valid ? null : (validation.error ?? 'Illegal position'));
  ...
  return (
    <Wrap className={className}>
        ...
        <ControlsColumn>
              ...
              {CASTLING_RIGHTS.map((right) => {
                ...
                return (
                  ...
                  >
                    ...
                    <ChipSide>{right === right.toUpperCase() ? 'white' : 'black'}</ChipSide>
                  </Chip>
                );
              })}
            </ChipRow>
          ...
          <Field>
            ...
            <Select
              ...
              onChange={(event) =>
                commitModel(
                  setEnPassant(model, event.target.value === '-' ? null : event.target.value),
                )
              }
            >
      ...
      {ghost !== null && (
        <Ghost
          aria-hidden="true"
          style={{ left: `${ghost.x}px`, top: `${ghost.y}px`, width: `${ghost.size}px`, height: `${ghost.size}px` }}
        >
...
const Chip = styled.button<{ $on: boolean; $warn?: boolean }>`
  ...
  padding: 0 ${(p) => p.theme.space.sm};
  border: 1px solid
    ${(p) => (p.$warn === true ? p.theme.color.danger : p.$on ? p.theme.color.accent : p.theme.color.border)};
  border-radius: ${(p) => p.theme.radius.md};
